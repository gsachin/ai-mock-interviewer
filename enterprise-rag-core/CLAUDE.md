...
- `prepopulate`: markdown corpus split on `## ` headings; deterministic chunk ids `{doc_id}:s{section}:c{chunk}`; marker gates (`--required-marker` / `--blocked-marker`); idempotent via `get_all` (reruns skip unless `--force`, which does `delete_by_parent` + re-upsert). Text-first core (`split_markdown_text` + `_ingest_markdown`) is shared with `register_bank` (Phase 4 runtime registration — both legs, no restart; force-replace deletes the keyword leg too via `KeywordStore.delete_by_parent`).
